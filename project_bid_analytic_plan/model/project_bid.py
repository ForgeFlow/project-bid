...
class ProjectBid(models.Model):
    ...
    def _prepare_cost_plan_lines(self, line):
        ...
        if line.name:
            ...
            raise ValidationError(
                _("All the material components have to include at least a"
        ...
        if account_id.active_analytic_planning_version != default_plan:
            raise ValidationError(
                _('The active planning version of the analytic account must '
                  'be %s. ')
                % (default_plan.name,))
    ...
    def _prepare_revenue_plan_lines(self):
        ...
        for bid in self:
            ...
            if not journal_id:
                raise ValidationError(
                    _('There is no expense journal defined '
