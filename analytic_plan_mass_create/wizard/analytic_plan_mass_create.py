...
class AnalyticPlanMassCreate(models.TransientModel):
    ...
    item_ids = fields.One2many("analytic.plan.mass.create.item", "wiz_id", "Items")
    ...
    def default_get(self, fields):
        ...
        if not analytic_account_ids:
            return res
        assert active_model == "account.analytic.account", "Bad context propagation"
        items = []
    ...
    def _prepare_analytic_line_plan(
        ...
        if a_type == "expense":
            general_account_id = product.product_tmpl_id.property_account_expense_id.id
            if not general_account_id:
                general_account_id = (
                    product.categ_id.property_account_expense_categ_id.id
                )
            journal_id = (
                product.expense_analytic_plan_journal_id
            ...
            general_account_id = product.product_tmpl_id.property_account_income_id.id
            if not general_account_id:
                general_account_id = (
            ...
            amount = item.revenue
            journal_id = (
                product.revenue_analytic_plan_journal_id
    ...
    def create_analytic_plan_lines(self):
        ...
        for item in wizard.item_ids:
            if item.delete_existing:
                line_ids = analytic_line_plan_obj.search(
                    [
                        ("account_id", "=", item.account_id.id),
                        ("version_id", "=", wizard.template_id.version_id.id),
                    ]
            ...
            if item.labor_cost:
                ...
                plan_line_id = analytic_line_plan_obj.create(plan_line_data_labor)
            ...
            if item.material_cost:
                ...
                plan_line_id = analytic_line_plan_obj.create(plan_line_data_material)
                res.append(plan_line_id.id)

            # Create Revenue
            if item.revenue:
                plan_line_data_revenue = self._prepare_analytic_line_plan(
                    wizard,
                    item,
                ...
                plan_line_id = analytic_line_plan_obj.create(plan_line_data_revenue)
                res.append(plan_line_id.id)
