...
class AnalyticPlanMassCreateTemplate(models.Model):
    ...
    material_cost_product_id = fields.Many2one(
        "product.product", "Default Material Cost Product", required=True
    )
    revenue_product_id = fields.Many2one(
        "product.product", "Default Material Revenue Product", required=True
    )
    version_id = fields.Many2one(
        "account.analytic.plan.version", "Default Planning Version", required=True
    )
